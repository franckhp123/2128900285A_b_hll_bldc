...
const uint8_t SELF_CW_PWM_OUT[6] = {BH_PWM&AH_PWM,BH_PWM      ,CH_PWM&BH_PWM,CH_PWM     ,AH_PWM&CH_PWM,AH_PWM     };
const uint8_t SELF_CW_P1_OUT[6]  = {CL_IO        ,AL_IO|CL_IO ,AL_IO        ,BL_IO|AL_IO,BL_IO        ,CL_IO|BL_IO};
uint8_t DriveData;
uint8_t int_flag=0;

/*********************************************************************************************************
  换相表载入:根据PWM模式和电机转向载入对应换相表
 输入: fnpaCommutationTab[]:电机换相表函数指针; ePwmMode:PWM模式; eMotorDir:电机转向
 输出: fnpaCommutationTab[]:电机换相表函数指针
 备注:当电机状态机从Standby切换到Boot时,当PwmAdcInt需要复位时,会根据当前设置的PWM模式以电机转向重新载入
**********************************************************************************************************/
void BLDC_CommutationTableLoad(FNP_VOID_T fnpaCommutationTab[],E_MOTOR_DIR_T eMotorDir)
{
   if(eMotorDir == eCW)      //正转  
   {
      fnpaCommutationTab[0] = NullPhase;
      fnpaCommutationTab[1] = UH_VL_HPwmLON;         
	  fnpaCommutationTab[2] = WH_UL_HPwmLON;
      fnpaCommutationTab[3] = WH_VL_HPwmLON;
	  fnpaCommutationTab[4] = VH_WL_HPwmLON;
	  fnpaCommutationTab[5] = UH_WL_HPwmLON;
	  fnpaCommutationTab[6] = VH_UL_HPwmLON;
   }
   else if(eMotorDir == eCCW) //反转
   {
      fnpaCommutationTab[0] = NullPhase;     
      fnpaCommutationTab[1] = WH_VL_HPwmLON;
	  fnpaCommutationTab[2] = UH_WL_HPwmLON;
      fnpaCommutationTab[3] = UH_VL_HPwmLON;
	  fnpaCommutationTab[4] = VH_UL_HPwmLON;
	  fnpaCommutationTab[5] = WH_UL_HPwmLON;
	  fnpaCommutationTab[6] = VH_WL_HPwmLON;    
   }
}

//根据初始位置得出下一个导通相
void BLDC_PhaseConducting(const FNP_VOID_T fnpaCommutationTab[],u8 u8Phase)
{
   switch(u8Phase)
   {
      case 1: fnpaCommutationTab[eWU]();  break;
	  case 3: fnpaCommutationTab[eWV]();  break;
	  case 2: fnpaCommutationTab[eUV]();  break;
	  case 6: fnpaCommutationTab[eUW]();  break;
	  case 4: fnpaCommutationTab[eVW]();  break;
	  case 5: fnpaCommutationTab[eVU]();  break;
	  default:break;
   }
}

/****************************************************************************************************
  根据设置电机转向来配置三相对应的AD通道,并配置导通相和反电势检测的AD采集顺序表
  描述:
  1.因为每个确定的相位所对应的需要采集的通道相和反电势也是确定的,所以可采用根据当前相位来查表方法实现
  2.本程序对正反转的操作是通过软件调换U和W相的AD通道和PWM引脚配置来实现,所以不同的转向对应的AD通道也是不同
  输入:eMotorDir 电机转向,*psPhaseAdChannel 三相的AD通道枚举指针
  输出:*psPhaseAdChannel 三相的AD通道枚举指针
  备注:当设置的电机转向发生变化时,需要使用这个函数
******************************************************************************************************/
void BLDC_PhaseAdChannelConfig(E_MOTOR_DIR_T eMotorDir,S_PHASE_AD_CHANNEL_T * psPhaseAdChannel)
{
   if(eMotorDir == eCW)   
   {
	  psPhaseAdChannel->eaConduction[0] = W_BEMF_ADCH; //当前导通相AD采集通道顺序
      psPhaseAdChannel->eaConduction[1] = U_BEMF_ADCH;
	  psPhaseAdChannel->eaConduction[2] = W_BEMF_ADCH;
      psPhaseAdChannel->eaConduction[3] = V_BEMF_ADCH;
	  psPhaseAdChannel->eaConduction[4] = V_BEMF_ADCH;
	  psPhaseAdChannel->eaConduction[5] = U_BEMF_ADCH; 
	
	  psPhaseAdChannel->eaBemf[0] = V_BEMF_ADCH; //反电势AD采集通道顺序
	  psPhaseAdChannel->eaBemf[1] = W_BEMF_ADCH;
	  psPhaseAdChannel->eaBemf[2] = U_BEMF_ADCH;
	  psPhaseAdChannel->eaBemf[3] = U_BEMF_ADCH;
	  psPhaseAdChannel->eaBemf[4] = W_BEMF_ADCH;
	  psPhaseAdChannel->eaBemf[5] = V_BEMF_ADCH;
   }
   else if(eMotorDir == eCCW)     
   {
      psPhaseAdChannel->eaConduction[0] = U_BEMF_ADCH; //当前导通相AD采集通道顺序
	  psPhaseAdChannel->eaConduction[1] = W_BEMF_ADCH;
	  psPhaseAdChannel->eaConduction[2] = U_BEMF_ADCH;
	  psPhaseAdChannel->eaConduction[3] = V_BEMF_ADCH;
	  psPhaseAdChannel->eaConduction[4] = V_BEMF_ADCH;
	  psPhaseAdChannel->eaConduction[5] = W_BEMF_ADCH;
	
	  psPhaseAdChannel->eaBemf[0] = V_BEMF_ADCH; //反电势AD采集通道顺序
	  psPhaseAdChannel->eaBemf[1] = U_BEMF_ADCH;
	  psPhaseAdChannel->eaBemf[2] = W_BEMF_ADCH;
	  psPhaseAdChannel->eaBemf[3] = W_BEMF_ADCH;
	  psPhaseAdChannel->eaBemf[4] = U_BEMF_ADCH;
	  psPhaseAdChannel->eaBemf[5] = V_BEMF_ADCH;
   }
}
...
/*
   ...
   TRDOER1=0xc8:开所有PWM; TRDOER1=0xFA:关PWM(只是pwm没了波形,中断还会触发)  
   
   TRDOER1:RD输出允许寄存器:0 允许输出 1为IO端口
     Bit7     Bit6     Bit5     Bit4     Bit3      Bit2     Bit1     Bit0
   TRDIOD1  TRDIOC1  TRDIOB1  TRDIOA1  TRDIOD0   TRDIOC0  TRDIOB0  TRDIOA0
   占空比   占空比   占空比   占空比   占空比    A0缓存   占空比    周期 
     UL       VL       UH       VH       WL                 WH
    P1.0     P1.1     P1.2     P1.3     P1.4               P1.5      
*/

void UH_VL_HPwmLON(void) //Debug OK    
{
...
//相位导通操作:根据电机转向导通对应相位,用在定位注脉冲中
void PwrDrv_UV_On(E_MOTOR_DIR_T eMotorDir)
{
   switch(eMotorDir)
   {
      case eCW:   UH_VL(); break;
	  case eCCW:  WH_VL(); break;
	  default:    break;
   }
}
...
void PwrDrv_UW_On(E_MOTOR_DIR_T eMotorDir)
{
   switch(eMotorDir)
   {
      case eCW:   UH_WL(); break;
	  case eCCW:  WH_UL(); break;
	  default:    break;
   }
}
void PwrDrv_VW_On(E_MOTOR_DIR_T eMotorDir)
{
   switch(eMotorDir)
   {
      case eCW:   VH_WL(); break;
	  case eCCW:  VH_UL(); break;
	  default:    break;
   }
}
void PwrDrv_VU_On(E_MOTOR_DIR_T eMotorDir)
{
   switch(eMotorDir)
   {
      case eCW:   VH_UL(); break;
	  case eCCW:  VH_WL(); break;
	  default:    break;
   }
}
void PwrDrv_WU_On(E_MOTOR_DIR_T eMotorDir)
{
   switch(eMotorDir)
   {
      case eCW:   WH_UL(); break;
	  case eCCW:  UH_WL(); break;
	  default:    break;
   }
}
void PwrDrv_WV_On(E_MOTOR_DIR_T eMotorDir)
{
   switch(eMotorDir)
   {
      case eCW:   WH_VL(); break;
	  case eCCW:  UH_VL(); break;
	  default:    break;
   }
}
...
{    uint8_t drvData;    TRDOER1=0xff;       drvData = P1;    drvData &= 0xc0;    P1=drvData;    }

//脉冲定位(初始位置检测)两相导通,3方向  
void BLDC_u8ImpulsePosition_2Phase3Dir_Fix(E_MOTOR_DIR_T eMotorDir)  
{
   g_sFlagEve.Bits.bMosCheckFlag = 1; //MOS开机自检开始     
   PwmDrv_PwmOutDisable(); //开始脉冲前关6管      
//第一轮:采集VW UV WU方向电流,WV VU UW用于抵消
   PwrDrv_VW_On(eMotorDir);
   Delay_us(IMPULSE_INJECT_TIME_uS);  
   AdcSampSingleChannelOnce(IPeak_ADCH); //读取电流脉冲AD值
...
//脉冲定位(初始位置检测)两相导通,6方向
u8 BLDC_u8ImpulsePosition_2Phase6Dir_Fix(E_MOTOR_DIR_T eMotorDir,u16 invaildThrval)
{
   const u8 aImpulseToPhaseTab[6] = {1,4,2,6,3,5}; //脉冲位置号对应的相位号  
   u8 impulseNum = 0;
   s16 aImpulse[6] = {0};
   #define iVW aImpulse[0]  
   #define iUV aImpulse[1] 
   #define iWU aImpulse[2]
   #define iWV aImpulse[3]
   #define iVU aImpulse[4]
   #define iUW aImpulse[5]

   PwmDrv_PwmOutDisable(); //开始前关6管   
//第一轮:采集VW UV WU方向电流,WV VU UW用于抵消   
   PwrDrv_VW_On(eMotorDir);    
   Delay_us(IMPULSE_INJECT_TIME_uS);
   iVW = AdcSampSingleChannelOnce(IPeak_ADCH); //读取电流脉冲AD值  
   PwmDrv_PwmOutDisable();  
   PwrDrv_WV_On(eMotorDir);
   Delay_us(IMPULSE_INJECT_TIME_uS);
   PwmDrv_PwmOutDisable();
   ...
   PwrDrv_UV_On(eMotorDir);
   Delay_us(IMPULSE_INJECT_TIME_uS);
   iUV = AdcSampSingleChannelOnce(IPeak_ADCH); //读取电流脉冲AD值 
   PwmDrv_PwmOutDisable();
   PwrDrv_VU_On(eMotorDir);
   Delay_us(IMPULSE_INJECT_TIME_uS);
   PwmDrv_PwmOutDisable();
   ...
   PwrDrv_WU_On(eMotorDir);
   Delay_us(IMPULSE_INJECT_TIME_uS);
   iWU = AdcSampSingleChannelOnce(IPeak_ADCH); //读取电流脉冲AD值 
   PwmDrv_PwmOutDisable();
   PwrDrv_UW_On(eMotorDir);
   Delay_us(IMPULSE_INJECT_TIME_uS);
   PwmDrv_PwmOutDisable();
